...
async function main() {
  ...
  // ARR convention (team-agreed 2026-08-15, matches Truc): a month runs 1st → 1st-of-next,
  // END-INCLUSIVE — a contract live by the 1st of next month counts, and one ending on the 1st of
  // next month STILL counts (it was active the whole month). So the live-side boundary is month-end+1
  // ($B+1) and the end-side boundary is month-end ($B): E>$B keeps a contract ending on the 1st.
  // Both clamped to TODAY for the in-progress month → that month stays on the W1 as-of-today finance
  // basis (finance tie preserved). See the ARR-boundary 1-pager in briefs/.
  const BND_MD = (r) => `MIN($B${r}+1,TODAY())`; // live/signed-side: on/before the 1st of next month
  const BND_ME = (r) => `MIN($B${r},TODAY())`;   // end-side: still active AFTER month-end
  const NOCHURN = `*(SOQL_Pull!$G$2:$G$${LAST}<>"Contracts Ended (Churned)")`;
  const activeAt = (r, extra = "") =>
    `=SUMPRODUCT((SOQL_Pull!$D$2:$D$${LAST}<=${BND_MD(r)})*(SOQL_Pull!$E$2:$E$${LAST}>${BND_ME(r)})${extra}${NOCHURN}*SOQL_Pull!$C$2:$C$${LAST})`;
  ...
  months.forEach((m, i) => {
    ...
    mom.push([
      ...
      `=SUMPRODUCT((SOQL_Pull!$D$2:$D$${LAST}<=${BND_MD(r)})*(SOQL_Pull!$E$2:$E$${LAST}>${BND_ME(r)})${NOCHURN}*SOQL_Pull!$S$2:$S$${LAST})`,
      `=C${r}-R${r}-S${r}`,
      ...
      `=SUMPRODUCT((SOQL_Pull!$T$2:$T$${LAST}<>"")*(SOQL_Pull!$T$2:$T$${LAST}<=${BND_MD(r)})*(SOQL_Pull!$E$2:$E$${LAST}>${BND_ME(r)})${NOCHURN}*SOQL_Pull!$C$2:$C$${LAST})`,
    ]);
  });
  ...
  const BND_WD = (r) => `MIN($A${r}+7,TODAY())`; // live-side: on/before next Monday (week runs Mon→Mon)
  const BND_WE = (r) => `MIN($A${r}+6,TODAY())`; // end-side: still active after the week's Sunday
  const wActive = (r, extra = "") =>
    `=SUMPRODUCT((SOQL_Pull!$D$2:$D$${LAST}<=${BND_WD(r)})*(SOQL_Pull!$E$2:$E$${LAST}>${BND_WE(r)})${extra}${NOCHURN}*SOQL_Pull!$C$2:$C$${LAST})`;
  ...
  weeks.forEach((wk, i) => {
    ...
    wow.push([
      ...
      `=SUMPRODUCT((SOQL_Pull!$D$2:$D$${LAST}<=${BND_WD(r)})*(SOQL_Pull!$E$2:$E$${LAST}>${BND_WE(r)})${NOCHURN}*SOQL_Pull!$S$2:$S$${LAST})`,
      `=B${r}-H${r}-I${r}`,
    ]);
  });
